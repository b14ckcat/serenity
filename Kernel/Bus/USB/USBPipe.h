...
public:
    ...
    static ErrorOr<NonnullOwnPtr<Pipe>> create(USBController const& controller, Type type, Direction direction, u8 endpoint_address, u16 max_packet_size, i8 device_address, size_t buffer_size = PAGE_SIZE, u16 poll_interval=0);
    ...
    // Async transfers
    ErrorOr<NonnullLockRefPtr<Transfer>> interrupt_transfer(size_t length, u16 ms_interval, usb_async_callback callback);
    void cancel_async_transfer(Transfer const& transfer);

private:
    friend class Device;

    Pipe(USBController const& controller, Type type, Direction direction, u8 endpoint_address, u16 max_packet_size, u16 poll_interval, i8 device_address, NonnullOwnPtr<Memory::Region> dma_buffer);
    ...
    u16 m_poll_interval { 0 };     // Polling interval (in frames)
