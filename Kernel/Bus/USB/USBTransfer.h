...
public:
    static ErrorOr<NonnullRefPtr<Transfer>> try_create(Pipe&, u16 length, NonnullOwnPtr<Memory::Region> &dma_buffer);
...
private:
    Transfer(Pipe& pipe, u16 len, NonnullOwnPtr<Memory::Region> &dma_buffer);
    Pipe& m_pipe;                                // Pipe that initiated this transfer
    NonnullOwnPtr<Memory::Region>& m_dma_buffer; // DMA buffer
